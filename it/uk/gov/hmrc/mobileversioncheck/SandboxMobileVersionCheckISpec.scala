...
class SandboxMobileVersionCheckISpec extends BaseISpec {
  ...
  def request: WSRequest = wsUrl(s"/mobile-version-check").withHeaders(acceptJsonHeader, mobileIdHeader)
  ...
  "POST /sandbox/mobile-version-check" should {
    "respect the sandbox headers and return true when the UPGRADE-REQUIRED control is specified" in {
      val response = await(request.withHeaders("SANDBOX-CONTROL" -> "UPGRADE-REQUIRED").post(toJson(DeviceVersion(iOS, "3.0.8"))))
    ...
    "respect the sandbox headers and return false when no control is specified" in {
      val response = await(request.post(toJson(DeviceVersion(iOS, "3.0.8"))))
    ...
    "respect the sandbox headers and return a 500 error when the ERROR-500 control is specified" in {
      val response = await(request.withHeaders("SANDBOX-CONTROL" -> "ERROR-500").post(toJson(DeviceVersion(iOS, "3.0.8"))))
